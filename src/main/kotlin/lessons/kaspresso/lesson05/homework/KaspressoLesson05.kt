package org.example.lessons.kaspresso.lesson05.homework

import java.time.LocalTime
import java.time.format.DateTimeFormatter

class KaspressoLesson05 {
}
...
class Schedule {
    ...
    operator fun invoke(fnc: Schedule.() -> Unit): Unit {
        fnc()
    }
    ...
    infix fun Pair<LocalTime, LocalTime>.schedule(lessonName: String){
        addSchedule(
            day ?: throw IllegalStateException("Не задан день недели"),
            ScheduleEntity(lessonName, first, second)
        )
    }
